...
async function launchBrowser(puppeteer, { headless = true, args = [] } = {}) {
  ...
  try {
    ...
    // open-agent-config: reuse installed Chrome when the bundled browser was
    // intentionally not downloaded. Windows already tried this channel above.
    if (process.platform !== 'win32') {
      try {
        return await puppeteer.default.launch({ channel: 'chrome', headless, args });
      } catch (systemError) {
        if (err && err.cause === undefined) err.cause = systemError;
      }
    }
