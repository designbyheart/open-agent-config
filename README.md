...
To enable the full catalog for both Claude Code and Codex in a project:

```bash
oac init --yes --targets=claude,codex --skills=all
oac doctor
```

This selects every current skill except the `_example` template. Full skill folders,
including scripts and references, are installed in `.claude/skills/` and
`.agents/skills/`; the instruction files contain a compact index for on-demand use.
Impeccable includes its pinned runtime scripts and specialist prompts. Install its
parser/browser dependencies after sync (Node 22.18+):

```bash
PUPPETEER_SKIP_DOWNLOAD=true npm ci --prefix .claude/skills/impeccable
PUPPETEER_SKIP_DOWNLOAD=true npm ci --prefix .agents/skills/impeccable
```

These commands reuse installed Chrome. Omit `PUPPETEER_SKIP_DOWNLOAD=true` if you
need Puppeteer to download its browser. They do not add dependencies to your app.
Automatic hooks are optional; the context loader directs manual detector runs.

Re-run the init command to include newly added catalog skills; `oac sync` refreshes
only the saved selection. Tool-dependent skills still require their referenced tools.

Codex installs now use `.agents/skills/`. For projects generated by older versions,
`oac sync` writes the current location and updates the index. Existing `.codex/skills/`
folders are left untouched to preserve custom content.
...
your-project/                 ← anywhere on disk
  ...
  .agents/skills/<name>/       ← skills copied in (Codex)
```
